...
#
    ...
    # /Model_Weights_saved'
    saved_path = '/home/user/catkin_ws/src/deep_drone/src/DDPG_for_ground_vehicle_tracking/Data_Saved'
    save_directory = os.path.join(os.getcwd(), saved_path)

    try:
        actor.model.load_weights(
            "/home/user/catkin_ws/src/deep_drone/src/DDPG_for_ground_vehicle_tracking/Data_Saved/Actor_weights/37_actor_weights.h5")
        actor.model_target.load_weights(
            "/home/user/catkin_ws/src/deep_drone/src/DDPG_for_ground_vehicle_tracking/Data_Saved/Actor_weights/37_actor_weights.h5")
        critic.model.load_weights(
            '/home/user/catkin_ws/src/deep_drone/src/DDPG_for_ground_vehicle_tracking/Data_Saved/Critic_weights/37_critic_model.h5')
        critic.model_target.load_weights(
            "/home/user/catkin_ws/src/deep_drone/src/DDPG_for_ground_vehicle_tracking/Data_Saved/Critic_weights/37_critic_model.h5")

        # critic.model_target.load_weights("/home/user/catkin_ws/src/deep_drone/src/Data_Saved/Actor_weights/219_critic_weights.h5")
        print("WEIGHTS LOAD CORRECTLY")
    except:
        print("ERR: WEIGHTS LOAD UNCORRECTLY")

    # return true if path is in an existing directory
    if not os.path.isdir(save_directory):
        os.makedirs(save_directory)
    os.chdir(save_directory)

   # Principal Training LOOP
    for ep in range(37, max_episode):
...
#                print('###############################')
            ...
            action_t = np.zeros([1, actions_dim])
            ...
            replay_buffer.add_experience(
                state_t, action_t[0], reward_t, state_t1, terminal)
...
#            print('reward_buff', reward_buff)
            ...
            # L = 1/N * sum(y_target - Q(si,ai|theta^Q)^2)
            loss = loss + \
                critic.model.train_on_batch(
                    [states_buff, actions_buff], y_target)
            ...
            distance_error = math.sqrt(
                state_t[0]*state_t[0] + state_t[1]*state_t[1])
...
#
            ...
            print('episode: {}, steps: {}, tot_rewards: {}, terminal: {}'.format(
        ...
        if (episode_check == desired_checking_episode):
            ...
            actor.model.save(action_model_name)
            ...
            actor.model.save_weights(
                action_save_weights_name+model_ext, overwrite=True)  # Save Weights
            with open(action_save_weights_name+model_ext2, "w") as outfile:
                # save Model Archutecture, not weights
                json.dump(actor.model.to_json(), outfile)

            critic_save_weights_name = 'Critic_weights/ %d_critic_weights' % (
                ep)
            critic.model.save_weights(
                critic_save_weights_name+model_ext, overwrite=True)
       ...
       #######################
        ...
        if (save_stats):
            ...
            if (episode_check == desired_checking_episode):
                ...
                np.savetxt(episode_name, episode_mat, delimiter=",")
                ...
                if (stat_step == True):
                    ...
                    # Nel post processing in matlab importare il vettore episode su asse x e fare plot con reward e distance su asse y
                    np.savetxt(distance_step_name,
                               distance_step_mat, delimiter=",")
                    np.savetxt(step_reward_name,
                               step_reward_mat, delimiter=",")
                    distance_step_mat = []
                    step_reward_mat = []
                    distance_step = []
                    step_reward = []
...
def start_test():
    ...
    load_path = '/home/user/catkin_ws/src/deep_drone/src/Data_Saved'  # /Model_Weights_saved'
...
#        actor_model = '/home/user/catkin_ws/src/deep_drone/src/Data_Saved/Actor_weights/%d_actor_model.h5' %(i)
#        critic_model = '/home/user/catkin_ws/src/deep_drone/src/Data_Saved/Critic_weights/%d_critic_model.h5' %(i)
#
...
#            print('ERROR: Model weight not succesfully')
        ...
        actor = ActorNetwork(env, sess)
        #critic = CriticNetwork(env,sess)
        actor_model = '/home/user/catkin_ws/src/deep_drone/src/Data_Saved/Actor_weights/%d_actor_model.h5' % (
            i)
        # critic_model = '/home/user/catkin_ws/src/deep_drone/src/Data_Saved/Critic_weights/%d_critic_model.h5' %(i)
        try:
        ...
        for ep in range(max_episode):
            ...
            while(terminal[0] == False):
                ...
                print('episode: {}, step: {},distance_error: {}, total_reward :{}'.format(
                    ep, step, distance_error, total_reward[0]))
            ...
            if (save_stats):
                ...
                if (episode_check == desired_checking_episode):
                    ...
                    episode_name = load_path + \
                        '/Test_Statistics/%d_test_episode.csv' % (ep)
                    episode_reward_name = load_path + \
                        '/Test_Statistics/%d_test_reward.csv' % (ep)
                    distance_name = load_path + \
                        '/Test_Statistics/%d_test_distance.csv' % (ep)
                    mean_reward_name = load_path + \
                        '/Test_Statistics/%d_test_mean_reward.csv' % (ep)
                    std_reward_name = load_path + \
                        '/Test_Statistics/%d_test_std_reward.csv' % (ep)
                    # Nel post processing in matlab importare il vettore episode su asse x e fare plot con reward e distance su asse y
                    np.savetxt(episode_name, episode_mat, delimiter=",")
                    np.savetxt(episode_reward_name,
                               ep_reward_mat, delimiter=",")
                    np.savetxt(mean_reward_name,
                               mean_reward_mat, delimiter=",")
