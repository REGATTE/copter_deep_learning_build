...
#      Define Goal Position in world frame
        ...
        # (x_des - x) and (y_des - y) and (vdes- vx) (vy_des -vy)
        self.num_states = 4
        self.num_actions = 2
        ...
        self.max_y = 15
        self.min_y = -15
        # max distance between the aerial vehicle and the ground target
        self.max_distance_vector = 10
    ...
    def _reset(self):
        ...
        # take the position of the drone respect ground
        initPoseStateData = [pose_error_x,
                             pose_error_y, vel_error_x, vel_error_y]
    ...
    # NB dovranno essere aggiunte anche le informazioni relative al veicolo terrestre
    def _step(self, action, step):
        ...
        if self.debug:
            print('vel_x:{}, vel_y: {}, yaw_vel: {}'.format(
        ...
        # stop the code in order to wait the upgrade of the simulation phisycs
        time.sleep(self.step_running)
        # it is possible to add imu inf, ecc
        poseData, imuData, velData, poseBox, velBox, altitude_yaw_vel_drone = self.takeEnvObservations()
        self.gazebo.pauseSim()
        ...
        reward, isTerminal = self.processingData(
            poseData, imuData, velData, poseBox, velBox, step)
        vel_drone_x = velData.vector.x
        vel_drone_y = velData.vector.y
        ...
        # take the position of the drone respect ground
        nextState = [pose_error_x, pose_error_y, vel_error_x, vel_error_y]
    ...
    # Function which takes information from the environment in gazebo
    def takeEnvObservations(self):
        ...
        while poseData is None:
            try:
                ...
                rospy.loginfo(
                    'Inable to reach the drone Pose topic. Try to connect again')

        imuData = None
        while imuData is None:
            try:
                imuData = rospy.wait_for_message(
                    '/ardrone/imu', Imu, timeout=5)
            except:
                rospy.loginfo(
                    'Inable to reach the drone Imu topic. Try to connect again')

        velData = None
        while velData is None:
            try:
                velData = rospy.wait_for_message(
                    '/fix_velocity', Vector3Stamped, timeout=5)
            except:
                rospy.loginfo(
                    "Inable to reach the drone Imu topic. Try to connect again")
        poseBox = None
        while poseBox is None:
            try:
                ...
                rospy.loginfo(
                    'Inable to reach the Box Pose topic. Try to connect again')
        velBox = None
        while velBox is None:
            try:
                velBox = rospy.wait_for_message(
                    '/mobile/cmd_vel', Twist, timeout=5)
            except:
                rospy.loginfo(
                    'Unable to reach the Box Velocity topic. Try to connect again')

        altitude_yaw_vel_drone = None
        while altitude_yaw_vel_drone is None:
            try:
                altitude_yaw_vel_drone = rospy.wait_for_message(
                    '/drone/cmd_vel', Twist, timeout=5)
            except:
                rospy.loginfo(
                    'Unable to reach the Box Velocity topic. Try to connect again')
    ...
    def processingData(self, poseData, imuData, velData, poseBox, velBox, step):
        ...
        a1 = +2.0 * (imuData.orientation.w * imuData.orientation.x +
                     imuData.orientation.y * imuData.orientation.z)
        a2 = 1.0 - 2.0 * (imuData.orientation.x*imuData.orientation.x +
                          imuData.orientation.y*imuData.orientation.y)
        roll = math.atan2(a1, a2)

        # pitch angle --> y axis rotation
        a3 = 2.0*(imuData.orientation.w * imuData.orientation.y -
        ...
        a4 = 2.0 * (imuData.orientation.w * imuData.orientation.z +
                    imuData.orientation.x * imuData.orientation.y)
        a5 = 1.0 - 2.0 * (imuData.orientation.y * imuData.orientation.y +
                          imuData.orientation.z * imuData.orientation.z)
        ...
        if pitch > self.max_incl or pitch < -self.max_incl:
            if self.debug:
                rospy.loginfo(
                    "Terminating Episode: Pitch value out of limits, unstable quad ----> "+str(pitch))
            ...
            if self.debug:
                rospy.loginfo(
                    "Terminating Episode: Roll value out of limits, unstable quad ----> "+str(roll))
            ...
            if self.debug:
                rospy.loginfo(
                    "Terminating Episode: X position value out of limits, unstable quad ----> "+str(poseData.pose.pose.position.x))
            ...
            if self.debug:
                rospy.loginfo(
                    "Terminating Episode: Y position value out of limits, unstable quad ----> "+str(poseData.pose.pose.position.y))
            ...
            if self.debug:
                rospy.loginfo(
                    "Terminating Episode: Distance value between the aerial and ground vehicle out of limits, unstable quad ----> "+str(distance))
            ...
            reward, goal_reached = self.getting_reward(
                poseData, imuData, velData, poseBox, velBox, step)  # To be designed the reward function
            if goal_reached:
                print('Goal Reached!')
                Terminal = [True]
...
#        print('y_drone', y_drone)
        ...
        distance_error = math.sqrt(
            pose_error_x*pose_error_x + pose_error_y*pose_error_y)

        vel_error_x = vel_box_x - vel_drone_x
        vel_error_y = vel_box_y - vel_drone_y
        vel_error_vehicles = math.sqrt(
            vel_error_x*vel_error_x + vel_error_y*vel_error_y)
